...
double complex evalf(double complex z) {
    return cpow(z, 9.0) - 3.0;
}

double complex evalDf(double complex z) {
    return 9.0 * cpow(z, 8.0);
}
...
void newton_basins(double l, double u, int p) {
    double complex z, sol[9] = {1.0, cexp(2.0 * M_PI * I / 9.0), cexp(4.0 * M_PI * I / 9.0), cexp(6.0 * M_PI * I / 9.0), 
                         cexp(8.0 * M_PI * I / 9.0), cexp(-2.0 * M_PI * I / 9.0), cexp(-4.0 * M_PI * I / 9.0), cexp(-6.0 * M_PI * I / 9.0), cexp(-8.0 * M_PI * I / 9.0)};
    double x[p], y[p];
    int C[p][p];
