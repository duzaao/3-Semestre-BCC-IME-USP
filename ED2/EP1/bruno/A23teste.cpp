...
for(int k = 0; k< Q+1; k++){
    ...
    if(x[0] == 'F'){
        ...
        int maior = arvore.tamanhoDaMaior(-1);
        cout << "Palavras com o maior tamanho (" << maior << "): ";
        arvore.imprimeTamanho(maior);
    }
    else if (x[0] == 'S' && x[1] == 'R') {
        int ok = 0 , ok2 = 0;
        int i;
        for(i = 0; i < 10 && !ok; i++){
            ok = arvore.tamanhoDaMaior(i);

        }
        arvore.analisaEcalculaSR(ok, --i);

    }
    else if (x[0] == 'V' && x[1] == 'D') {
        int a = arvore.VD();
        int b =  arvore.VD2(a);
        arvore.imprimeVD(a,b);
